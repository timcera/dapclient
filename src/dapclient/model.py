"""This is the dapclient data model, an implementation of the Data Access Protocol
...
from dapclient.lib import _quote, decode_np_strings, tree, unquote, walk
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            if hasattr(self, "_original_data_args"):
                from dapclient.handlers.dap import BaseProxyDap4
...
class DatasetType(StructureType):
    ...
    def _resolve_batch(self, batch_promise):
        from dapclient.handlers.dap import UNPACKDAP4DATA
        ...
        if not self._checksums:
            warnings.warn(
                ...
                "be in the next version of dapclient. Setting `checksums=True`",
                stacklevel=2,
            )
...
class SequenceType(StructureType):
    ...
    def items(self):
        # This method should be removed in dapclient 3.4
        for key in self._visible_keys:
            yield (key, self[key])
