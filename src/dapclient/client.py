...
from dapclient.handlers.dap import (
    UNPACKDAP4DATA,
    DAPHandler,
    StreamReader,
    unpack_dap2_data,
)
from dapclient.lib import DEFAULT_TIMEOUT, Failure, _is_retryable, encode, tqdm, walk
from dapclient.model import BaseType, BatchPromise, DapType
from dapclient.net import GET, create_session, extract_session_state, get_session
from dapclient.parsers.das import add_attributes, parse_das
from dapclient.parsers.dds import dds_to_dataset
from dapclient.parsers.dmr import DMRParser, dmr_to_dataset
...
def open_url(
    ...
    """
    Open a remote OPeNDAP URL, or a local (wsgi) application returning a dapclient
    dataset.
    ...
    protocol : str | bool (Default: None)
        ...
        `None`. Other  options are 'dap2' and 'dap4'. When None, dapclient attempts to
...
def consolidate_metadata(
    ...
    safe_mode : bool, optional (default=True)
        ...
        empty dapclient dataset. dimensions names and sizes are checked for
    ...
    for item in items:
        ...
        if granule:
            ...
            warnings.warn(
                ...
                "the parameters are correct. If you think this is an issue with dapclient"
                " or the cmr, consider opening an issue on the dapclient github repository"
            )
...
def register_all_for_batch(ds, Variables, checksums=True) -> None:
    """
    Used to register all dimension array when dapclient
...
def fetch_batched(ds, Variables) -> None:
    """
    Helper function that fetched dimensions within a dapclient dataset
    ...
    Parameters:
    ----------
        ds: dapclient dataset | GroupType (dap4)
        Variables: list
...
def fetch_consolidated(ds, cache_urls=None, checksums=True) -> None:
    ...
    headers. It also requires that the dapclient dataset associated with the BaseType
