...
from dapclient.model import BaseType, DatasetType, SequenceType
...
def test_iter(simple_array, simple_object):
    assert [tuple(row) for row in simple_array] == [
        (0, 1, 10.0),
        (1, 2, 20.0),
        (2, 3, 30.0),
        (3, 4, 40.0),
    ...
    assert list(simple_array["byte"]) == [0, 1, 2, 3, 4, 5, 6, 7]
    ...
    assert list(simple_object["byte"]) == [0, 1, 2, 3, 4, 5, 6, 7]


def test_dtype(simple_array, simple_object):
    assert simple_array.dtype == np.dtype(
        [("byte", "i1"), ("int", "<i4"), ("float", "<f4")]
    )
    assert simple_array["int"].dtype == "<i4"

    assert simple_object.dtype == np.dtype(
        [("byte", "<i8"), ("int", "<i8"), ("float", "<f8")]
    )
...
def test_projection(simple_array, simple_object):
    ...
    assert list(simple_array["byte"][1::2]) == [1, 3, 5, 7]
    assert list(simple_array[1::2]["byte"]) == [1, 3, 5, 7]
    ...
    assert list(simple_object["byte"][1::2]) == [1, 3, 5, 7]
    assert list(simple_object[1::2]["byte"]) == [1, 3, 5, 7]
...
def test_combined(simple_array, simple_object):
    ...
    filtered = filtered[::2]
    assert [tuple(row) for row in filtered] == [(2, 3, 30.0), (4, 5, 50.0)]
...
def test_combined_other(simple_array, simple_object):
    ...
    assert [tuple(row) for row in filtered] == [(1, 10.0), (2, 20.0)]
...
@pytest.fixture
def nested_data():
    shallow_data = [(1, 1, 1), (2, 4, 4), (3, 6, 9), (4, 8, 16)]
    deep_data = [
        [(10, 11, 12), (21, 22, 23)],
        [(15, 16, 17)],
        [],
...
def test_nested_iter(nested_data, nested_object):
    assert [tuple(row) for row in nested_object] == nested_data
...
def test_nested_iter_nested_deep_child(nested_data, nested_object):
    expected = [[col[0] for col in row[3]] for row in nested_data]
...
def test_nested_selection(nested_data, nested_object):
    ...
    expected = [tuple(row) for row in nested_data if row[0] > 2]
...
def test_nested_projection(nested_data, nested_object):
    ...
    expected = [
        tuple(row)
        for row_id, row in enumerate(nested_data)
...
def test_nested_combined(nested_data, nested_object):
    filtered = [tuple(row) for row in nested_data if row[0] > 2]
    filtered = [tuple(row) for row in filtered if row[0] < 4]
    filtered = filtered[::2]
